...
int
PmPosture::ik_body( PmConstraint const& c )
{
	if ( this->getBody() )
		return ik_body( c, this->getMask() & c.getJointMask(this->getBody()) );

	return ik_body( c, this->getMask() );
}

int
PmPosture::ik_body( PmConstraint const& c, PmMaskType m )
{
	constraint = c;
	...
	for( i=0; i<c.getNumEntity(); i++ )
		if ( c.entity[i].mask & PM_C_ORIENTATION )
		{
...
computeJacobian()
{
	...
	for( i=0, ii=0; i<constraint.getNumEntity(); i++ )
	{
		...
		if ( c.mask & PM_C_POSITION )
		{
			dv = c.value.getTranslation() - endTrans;
			dist += dv%dv;
		...
		if ( c.mask & PM_C_ORIENTATION )
		{
			dq = difference( c.value.getRotation(), endRot );
			dist += dq%dq;
