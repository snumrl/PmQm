...
//----------------------------------------------------------------------//
//
//  For acclaim files that are generated from Kaydara Mocap (Racket)
//
#define	PM_MOTION_NUM_JOINT_IN_ASF_FILE	 1
#define PM_MOTION_NUM_DUMMY				 0
...
void
ConvertASF2ACTOR_Racket(char* asf_filename, char* skeleton_filename, m_real scale)
{
...
int
PmLinearMotion::saveAMC_Racket( char* name )
{
...
int
PmLinearMotion::openAMC_Racket( char* name, m_real scale, int subsample )
{
	assert( this->getBody() );
